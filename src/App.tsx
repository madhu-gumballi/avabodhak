...
// import { FlowTransport } from './components/FlowTransport';
...
export default function App() {
  ...
  const text = (linesFile as TextFile);
  const ttsEnabled = isTTSEnabled();
  const [lang, setLang] = useState<Lang>(() => {
    try {
      const raw = localStorage.getItem('lang');
      const v = (raw as Lang) || 'iast';
      return (v === 'mr' || v === 'ben' || v === 'mal') ? 'iast' : v;
    } catch { return 'iast'; }
  });
  const [lang2, setLang2] = useState<Lang | ''>(() => {
    try {
      const raw = localStorage.getItem('lang2');
      const v = (raw as Lang) || 'knda';
      return (v === 'mr' || v === 'ben' || v === 'mal') ? '' : v;
    } catch { return 'knda'; }
  });
  ...
  // useWordFlow no longer needs TTS player - it's just for navigation now
  const flow = useWordFlow(text.lines as Line[], lang, undefined);

  // TTS playback state (separate from flow state)
  ...
  const uiPlaying = flow.state.playing || holdingGroup;
  ...
  // Global keyboard shortcuts (reading mode only): Space toggles, Left/Right navigate by lines
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' || e.code === 'Space') { e.preventDefault(); flow.toggle(); setOverlayVisible(true); return; }
      if (e.key === 'ArrowLeft') {
        e.preventDefault();
  ...
  const handleTTSPlayback = useCallback(async () => {
    ...
    setTtsPlaying(true);
    try {
      // Await the entire TTS playback chain so it stays within the
      // async call stack of the user gesture (helps with autoplay rules).
          ...
          <Toolbar sx={{ gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Select size="small" value={lang} onChange={(e: SelectChangeEvent) => { const newLang = e.target.value as Lang; setLang(newLang); analytics.languageChange(newLang); ensurePlayPauseReady(); }} sx={{ minWidth: isSmall ? 72 : 140 }}>
                {(['deva', 'knda', 'iast', 'tel', 'tam', 'guj', 'pan'] as Lang[]).map((code) => (
              ...
              <Select size="small" value={lang2 || ''} onChange={(e: SelectChangeEvent) => { const newLang = (e.target.value || '') as any; setLang2(newLang); if (newLang) analytics.languageChange(`${newLang}_secondary`); ensurePlayPauseReady(); }} sx={{ minWidth: isSmall ? 72 : 140 }} displayEmpty>
                ...
                {(['deva', 'knda', 'iast', 'tel', 'tam', 'guj', 'pan'] as Lang[]).filter(code => code !== lang).map((code) => (
              ...
              <Tooltip title={viewMode === 'reading' ? 'Practice Mode' : 'Reading Mode'}>
                <IconButton
                  ...
                  onClick={() => {
                    ...
                    // Auto-pause when switching to practice mode
                    if (newMode === 'practice' && flow.state.playing) {
                      flow.pause();
              ...
              <Tooltip title={viewMode === 'puzzle' ? 'Reading Mode' : 'Puzzle Mode'}>
                <IconButton
                  ...
                  onClick={() => {
                    ...
                    // Auto-pause when switching to puzzle mode
                    if (newMode === 'puzzle' && flow.state.playing) {
                      flow.pause();
                      analytics.playAction('pause');
                    }
                  ...
                  <Box>
                    ...
                    <div className="flex items-center justify-between mb-2 gap-2">
                      ...
                      <div className="flex items-center flex-shrink-0">
                        {/* Paused pill: only when explicitly paused (not while group-hold animation is driving) */}
                        {!flow.state.playing && !holdingGroup && (
                      ...
                      <Box
                        ...
                        onTouchEnd={(e) => {
                          ...
                          if (Math.abs(deltaX) > 50 && deltaTime < 300 && Math.abs(deltaX) > Math.abs(deltaY) * 1.5) {
                            // Auto-pause on swipe (hybrid mode)
